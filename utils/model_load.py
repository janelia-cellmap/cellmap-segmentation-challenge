...
# load_latest checks to see if there are any checkpoints to load in
def load_latest(search_path, model):
    ...
    if checkpoint_files:
        ...
        # Saves the latest checkpoint into a variable
        newest_checkpoint = checkpoint_files[0]

        # Loads the most recent checkpoint into the model and prints the file path
