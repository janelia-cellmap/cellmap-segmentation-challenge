...
class TestPredictFiltering:
    ...
    def test_tensor_output_filtering(self):
        ...
        outputs = {"output": torch.randn(2, 4, 64, 64, 64)}  # (B, C, D, H, W)
...
class TestPredictEndToEnd:
    ...
    def test_predict_with_test_crops_filters_classes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch(
                    "cellmap_segmentation_challenge.predict.get_test_crops"
                ) as mock_get_crops,
                patch(
                    "cellmap_segmentation_challenge.predict.get_test_crop_labels"
                ) as mock_get_labels,
                patch(
                    "cellmap_segmentation_challenge.predict.load_safe_config"
                ) as mock_load_config,
                patch(
                    "cellmap_segmentation_challenge.predict.get_model"
                ) as mock_get_model,
                patch(
                    "cellmap_segmentation_challenge.predict.CellMapDatasetWriter"
                ) as mock_writer_class,
                patch(
                    "cellmap_segmentation_challenge.predict._predict"
                ) as mock_predict_func,
            ):
                ...
                mock_get_labels.return_value = [
                    "mito",
                    "er",
                ]  # Only these 2 labels for this crop
