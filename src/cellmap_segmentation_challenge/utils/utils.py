        ...
        for k, v in outputs.items():
            if num_channels_per_class is not None:
                ...
                if v.shape[1] != expected_channels:
                    raise ValueError(
                        f"Number of output channels ({v.shape[1]}) does not match "
                        f"expected channels ({expected_channels}) for {len(classes)} "
                        f"classes with {num_channels_per_class} channels per class."
                    )
                structured[k] = {
                    class_name: v[
                        :,
