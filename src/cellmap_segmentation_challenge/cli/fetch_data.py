...
def fetch_crops_cli(crops: str, raw_padding: str, dest_dir: str, access_mode: str):
    ...
    for crop in crops_parsed:
        ...
        if em_source_group is None:
            log.info(f'No EM data found at any of the possible URLs. No EM data will be fetched.')        
            break 
        ...
        if em_source_group is not None:         
            ...
            for _, group in gt_source_group.groups():
                try:
                    ...
                    continue
                raise ValueError(f'No multiscale groups found in {gt_source_url}. No EM data can be fetched.')
            ...
            _, (base_scale, _) = transforms_from_coords(em_source_arrays_sorted[0][1].coords, transform_precision=4)
            for key, array in em_source_arrays_sorted:
                _, (current_scale, _) = transforms_from_coords(array.coords, transform_precision=4)
                relative_scale = base_scale.scale[0] / current_scale.scale[0]
                ...
                em_group_inventory += tuple(map(lambda v: '/'.join([key, v]), get_chunk_keys(em_source_group[key], slices_padded)))
